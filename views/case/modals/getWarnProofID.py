...
class getWarnProofID(discord.ui.Modal):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        try:
            await interaction.response.send_message(content=f"This is {latestUserName}'s #{self.children[0].value}'s warn proof", file=discord.File(f"images/proofs/warns/{latestUserName}_warn{self.children[0].value}"), ephemeral=True)
        except:
